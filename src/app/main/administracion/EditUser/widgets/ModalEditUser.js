  ...
  const ApiPatch = () => {
    ...
    if (isEqual) {
      ...
      setLoading(false);
        ...
        setMsgAlert({msgResp: true,msgText:"Error, no ha realizado ningun cambio!!",msgError:true});
        setTimeout(() => {
          handleCloseSecond();
        },1200);
    } else {
      ...
      axios
        ...
        .catch((error) => {
          setLoading(false);
          setMsgAlert({msgResp: true,msgText:"Error, no ha se ha modificado el usuario",msgError:true});
          setTimeout(() => {
            handleCloseSecond();
          },2500);
        });
    ...
    <DialogContent dividers={scroll === 'paper'}    >
      <div className="flex flex-col w-full">
        ...
        <Card>
          <CardHeader
            ...
            avatar={
              <Checkbox
                ...
                checked={checked.length === clientDisp.length && checked.length !== 0}
                // onChange={(event) => {
                //   setCheckedBlue(
                //     event.target.checked
                //   );
                ...
                indeterminate={ checked.length !== clientDisp.length && checked.length !== 0 }
          ...
          />
          <Divider />
          <List sx={{ height:'300px',width: '100%', maxWidth: 300, bgcolor: 'background.paper', overflow: 'auto'}}>
          ...
          <CardHeader
            ...
            avatar={
              <Checkbox
                ...
                checked={checkeddos.length === clientAsig.length && checkeddos.length !== 0}
                // onChange={(event) => {
                //   setCheckedBlue(
                //     event.target.checked
                //   );
                ...
                indeterminate={ checkeddos.length !== clientAsig.length && checkeddos.length !== 0 }
                disabled={!checkeddos.length === 0}
                inputProps={{
                  'aria-label': 'all items selected',
                }}
          ...
          {clientAsig.map((value) => {
            ...
            return (
              <ListItem
                ...
                disablePadding
              >
                <ListItemButton role={undefined} onClick={handleToggledos(value)} dense>
                  <ListItemIcon>
                    <Checkbox
                      edge="start"
